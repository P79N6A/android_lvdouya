...
public class LoopView extends View {
    ...
    public void setSelectColor(@ColorInt int color) {
        customPaintAColor = color;
    }

    private int customPaintBColor;

    public void setNormalColor(@ColorInt int color) {
        customPaintBColor = color;
    }
...
//        paintA = new Paint();
        if (customPaintAColor != 0) {
            paintA.setColor(customPaintAColor);
        } else {
            paintA.setColor(context.getResources().getColor(R.color.color_calendar_text_no_state));
        }
        paintA.setAntiAlias(true);
        paintA.setTypeface(Typeface.MONOSPACE);
        paintA.setTextSize(textSize);
//        paintB = new Paint();
        if (customPaintBColor != 0) {
            paintB.setColor(customPaintBColor);
        } else {
